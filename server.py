    ...
    for uid in users:
        ...
        if container_rows_html:
            ...
            containers_block = (
                ...
                f'<a href="/auth/discover?user_id={uid}" style="color:#1a73e8;font-size:13px">↻ Discover containers</a></div>'
            )
        else:
            containers_block = ""

        cards += (
            f'<div style="background:#12121e;border:1px solid rgba(26,115,232,.2);border-radius:12px;'
...
.logo-bar{{margin-bottom:32px;display:flex;align-items:center;gap:16px;padding-bottom:24px;border-bottom:1px solid rgba(26,115,232,.12)}}
.logo-bar img{{height:42px;border-radius:10px}}
.subtitle{{color:#1a73e8;font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase}}
.card{{background:#12121a;border:1px solid rgba(26,115,232,.15);border-radius:12px;padding:24px;margin-bottom:14px}}
...
.stat{{background:#0e0e1a;border:1px solid rgba(26,115,232,.1);border-radius:8px;padding:12px 18px;flex:1;min-width:80px}}
...
input[type=text]{{padding:10px 14px;background:#0e0e18;border:1px solid rgba(26,115,232,.3);border-radius:7px;color:#e8e8f4;font-size:14px;width:100%;max-width:280px}}
input[type=text]:focus{{outline:none;border-color:#1a73e8}}
input[type=text]::placeholder{{color:#404060}}
.btn{{display:inline-block;padding:10px 22px;background:linear-gradient(135deg,#1557b0,#1a73e8);color:white;text-decoration:none;border-radius:7px;border:none;cursor:pointer;font-size:14px;font-weight:600;white-space:nowrap}}
